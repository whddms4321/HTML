...
1. 프로그래밍
  ...
  프로그램이란? : 컴퓨터가 수행해야 할 명령어들을 작성해 둔 것들
  
프로그래밍언어

  - 프로그램을 작성할 때 사람과 컴퓨터 사이에 대화를 가능하게 해주는 컴퓨터가 이해 할 수 있는 언어
  

2. JAVA 

  - 1995년에 제임스 고슬링과 그 동료들이 만든 언어
  - 전기밥통과 같은 가전제품을 제어하기 위한 언어로 고안
  - 안드로이드 기반 언어
...
![imgJAVACapture1](./imgs/JAVACapture1)

  - __자바는 사용자/개발 목적에 따라 다양한 플랫폼 제공__

# 이클립스

1. 이클립스를 이용한 자바 코드 작성

  1. Project 생성
    - 하나의 큰 작업(하나의작품)으로 여러 개의 패키지가 존재하는 공간
  2. Package 생성
  ...
  5. Method 내부에 소스코드 장성
  6. 컴파일 및 실행(Ctrl+ F11)
  
2. 작성 규칙

  1. 소스코드는 메소드의 불록안에서 작성({}을 의미)
  2. 클래스 안에 작성된 public static void main(String[] args)는 메인 메소드 또는 실행 메소드라고 <br> 부르며 하나의 프로젝트당 한개만 만들어서 사용
  3. System.out.println("Hi")는  "" 안에 값을 프로그램 안에서 출력 하는 명령어
  
3. 표기법
  
  1. 카멜 표기법
    - 소문자로 시작해 뒷단어의 첫글자를 대문자로 작성하는 방식 : ex) noticeNumber;
  2. 파스칼 표기법
    - 첫 글자도 대문자 표기 : ex)NoticeNumber;
  3. 
...
## PL/SQL H2   
  ...
  1. PL/SQL의 변수의 종류   
       ...
       - 상수 : 일반변수와 유사하나 CONSTANT 키워드가 자료형 앞에 붙고 선언시 값을 할당해 주어야함. <br> 값 지정하면 변경 불가능
...
2. __jdbc의 종류__
  ...
  4. **PreparedStatement**    
     ...
     - SQL 문장이 미리 컴파일 되고, 실행시간 동안 인수 값을 위한 공간을 확보 <br> 할 수 있다는 점에서 Statement 와 다름
  ...
  5. **ResultSet**
     ...
     - ResultSet은 SQL 질의에 의해 생성된 결과를 담고 있으며 '커서(cursor)0'를  <br> 이용하여 특정 행에 대한 참조를 조작       
        
  6. __jdbc의 작성 절차__

    1. 드라이버 등록
    2. Connection객체생성
    3. Statement객체생성 - >preparedStatement 일때 위치홀더 사용으로 <br> sql을 정의 캐쉬를 사용해서 연산속도 상승,보안성 높아짐 
    4. 쿼리문 전송 및 결과 수신
    5. 결과 처리
    6. 자원 반환    
  ...
  7. **single ton(싱글톤) 패턴**  
    ...
    - 이때 만들어진 객체를 불러다 쓰기위해 __Static__ 형태를 사용한다.
 ...
 8. __MVC__
   ...
   3. 논리 데이터 모델링에서 제 1정규화로 이뤄어지는 3가지     
